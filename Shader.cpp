...
ID3D11VertexShader* LoadVertexShader(std::string shaderName, IEngine * engine)
{
	...
	HRESULT hr = engine->GetDevice()->CreateVertexShader(byteCode.data(), byteCode.size(), nullptr, &shader);
	if (FAILED(hr))
	{
		return nullptr;
	}
...
ID3D11GeometryShader* LoadGeometryShader(std::string shaderName, IEngine * engine)
{
	// Open compiled shader object file
	std::ifstream shaderFile(shaderName + ".cso", std::ios::in | std::ios::binary | std::ios::ate);
	if (!shaderFile.is_open())
	...
	HRESULT hr = engine->GetDevice()->CreateGeometryShader(byteCode.data(), byteCode.size(), nullptr, &shader);
	if (FAILED(hr))
	{
		return nullptr;
	}
...
ID3D11PixelShader* LoadPixelShader(std::string shaderName, IEngine * engine)
{
	...
	HRESULT hr = engine->GetDevice()->CreatePixelShader(byteCode.data(), byteCode.size(), nullptr, &shader);
	if (FAILED(hr))
	{
		return nullptr;
	}
...
ID3D11Buffer* CreateConstantBuffer(int size, IEngine * engine)
{
	...
	HRESULT hr = engine->GetDevice()->CreateBuffer(&cbDesc, nullptr, &constantBuffer);
	if (FAILED(hr))
	{
		return nullptr;
	}
