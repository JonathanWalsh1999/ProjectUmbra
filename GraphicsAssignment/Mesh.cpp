...
Mesh::Mesh(const std::string& fileName, IEngine * engine = nullptr, bool requireTangents /*= false*/)
{
	...
	hr = myEngine->GetDevice()->CreateBuffer(&bufferDesc, &initData, &mVertexBuffer);
	...
	hr = myEngine->GetDevice()->CreateBuffer(&bufferDesc, &initData, &mIndexBuffer);
						...
						const std::string& psShaderFile, const std::string vsShaderFile)
{
	...
	newModel->SetTextureFile(textureFile);

	ID3D11VertexShader* modelVSShader = nullptr;
	ID3D11PixelShader* modelPSShader = nullptr;
	ID3D11ShaderResourceView* diffuseSpecularMapSRV = nullptr;
	ID3D11Resource* diffuseSpecularMap = nullptr;

	//load shaders
	//modelVSShader = LoadVertexShader(vsShaderFile);
	//modelPSShader = LoadPixelShader(psShaderFile);
	//newModel.associatedVSShader = modelVSShader;
	//newModel.associatedPSShader = modelPSShader;
	...
	newModel->SetDiffuseMap(diffuseSpecularMap);
	newModel->SetDiffuseSRVMap(diffuseSpecularMapSRV);
